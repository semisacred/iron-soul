...
Float Property IdlePollSeconds = 2.00 Auto
...
String Function _RespawnUsedStorageKey(String guid)
    ; Co-save storage disabled (JSON-only state)
    return ""
EndFunction
...
; Recovery is conservative to avoid cross-character mixups:
;   - If the player name is valid, prefer GUIDForName:<name>.
;   - Otherwise, fall back to ActiveGUID if its stored signature/level are compatible.
String Function RecoverGuidFromJson(Actor player, String curSig, Int curLevel)
    ...
    if IsValidPlayerName(name)
        String g1 = JsonUtil.GetStringValue(SharedPath, "GUIDForName:" + name, "")
        String g2 = JsonUtil.GetStringValue(MirrorPath, "GUIDForName:" + name, "")
        if g1 != "" && _GuidCandidateMatches(player, g1, curSig, curLevel, name)
            return g1
        endif
        if g2 != "" && _GuidCandidateMatches(player, g2, curSig, curLevel, name)
            return g2
        endif
    endif

    ; --- Fallback path: last active GUID (weak) ---
    String a1 = JsonUtil.GetStringValue(SharedPath, "ActiveGUID", "")
    String a2 = JsonUtil.GetStringValue(MirrorPath, "ActiveGUID", "")
    if a1 != "" && _GuidCandidateMatches(player, a1, curSig, curLevel, name)
        return a1
    endif
    if a2 != "" && _GuidCandidateMatches(player, a2, curSig, curLevel, name)
        return a2
    endif
...
Bool _deferredKillForSoulRevive = False
...
;=========================
;== PER-TICK IDENTITY CACHE ==
;=========================
Actor _tickPlayer = None
Bool _tickGuidSet = False
String _tickGuid = ""
String _tickDeathsStorageKey = ""
String _tickDeathKeyById = ""

Function _BeginTickCache(Actor player)
    _tickPlayer = player
    _tickGuidSet = False
    _tickGuid = ""
    _tickDeathsStorageKey = ""
    _tickDeathKeyById = ""
EndFunction

String Function _GetTickGuid()
    if _tickGuidSet
        return _tickGuid
    endif
    if !_tickPlayer
    ...
    _tickGuid = GetCharacterGUID(_tickPlayer)
    _tickGuidSet = True
    return _tickGuid
EndFunction

String Function _GetTickDeathsStorageKey(String guid)
    if guid == ""
        return ""
    endif
    if _tickDeathsStorageKey != "" && _tickGuid == guid
        return _tickDeathsStorageKey
    endif
    _tickDeathsStorageKey = _DeathsStorageKey(guid)
    return _tickDeathsStorageKey
EndFunction

String Function _GetTickDeathKeyById(String guid)
    if guid == ""
        return ""
    endif
    if _tickDeathKeyById != "" && _tickGuid == guid
        return _tickDeathKeyById
    endif
    _tickDeathKeyById = GetDeathKeyById(guid)
    return _tickDeathKeyById
EndFunction
...
Function SetEssentialAndStartQuestIfNeeded(Actor player, String guid, String name)
    ...
    ; and keep the player essential (respawn/bleedout based). If not, stop the respawn quest
    ; (if any) and make the player non-essential so logged deaths can finalize / ragdoll.
...
Function HandlePlayerDying(Actor akKiller)
	...
	; No free respawn remaining: ensure respawn quest is stopped and the player is non-essential.
	if RespawnQuest && RespawnQuest.IsRunning()
		RespawnQuest.Stop()
	endif
	player.GetActorBase().SetEssential(False)
	...
	if souls > 0
		if !_revivePending
			...
			; Prevent Skyrim from finalizing death while we wait for the revive job (player is non-essential in this mode)
			player.StartDeferredKill()
			_deferredKillForSoulRevive = True
			_revivePending = True
			_reviveConsumedSoul = False
			_postResurrectRecastPending = False
...
String Function GetRespawnFlavorLine()
    ...
    ;
    ; Build a compact pool of non-empty lines so we never "randomly" hit empty slots.
...
Bool Function HandleStartupMessage(Actor player)
    ...
    String pName = player.GetDisplayName()
    if IsValidPlayerName(pName)
        Int dName = _SetStringIfChanged(SharedPath, "LastKnownName:" + guid, pName, MirrorPath, "LastKnownName:" + guid, pName)
        if dName != 0
            _MarkAuthDirty((dName % 2) == 1, dName >= 2)
        endif
    endif
    ...
    if _pendingDisableRespawn
        ...
        if !player.IsBleedingOut() && !player.IsDead()
            if RespawnQuest && RespawnQuest.IsRunning()
                ...
                RespawnQuest.Stop()
                ; Respawn has completed / been consumed: make the player non-essential so true deaths can finalize.
                player.GetActorBase().SetEssential(False)
            else
    ...
    if _revivePending
        ...
        if !_reviveConsumedSoul
            ...
            if soulsNow > 0
                ...
                ; Ensure any deferred-kill state is cleared before allowing true death to finalize.
                if _deferredKillForSoulRevive
                    player.EndDeferredKill()
                    _deferredKillForSoulRevive = False
                endif
                TrueDeathAndQuit(player)
                return True
            endif
        ...
        ; Now that we're safely above 0 HP, allow death finalization again.
        if _deferredKillForSoulRevive
            player.EndDeferredKill()
            _deferredKillForSoulRevive = False
        endif
