...
Function HandleLoadMessage(Actor player)
    ; Job A: timed load‑game message.  Uses externalised strings for
    ; flavour and defers all customisation to config files.
            ...
            if !suppress
                ...
                if !suppress
                    ...
                    if deaths >= _maxLives
                        ...
                        ; When the free respawn has been consumed, show a fatal warning notice
                        if used > 0 && !_disableFatalReminderMessage
                            Debug.Notification("Your recent trial has left you shaken.")
                        else
                            ; Otherwise show a simple status notification with name and death count
                            String msg = "Your Iron Soul endures. Name: " + name + " Deaths: " + deaths
                            ; If logging is enabled include the GUID for easier debugging
                            if _logEnabled
                                msg = msg + " GUID=" + guid
                            endif
