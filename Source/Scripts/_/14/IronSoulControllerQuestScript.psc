...
Float Property DragonSoulBleedoutVFXHoldSeconds = 2.0 Auto
...
Float _reviveHoldUntil = 0.0
Bool _reviveEarlyVFXPlayed = False
...
Function RescheduleIfJobsRemain()
	...
	if _pendingDisableRespawn || _revivePending || _pendingLoadMessage
		QueueUpdate(0.10)
	else
...
Bool Function TryResolveNoRespawnDeath(Actor player, String guid, Int used, String context)
    ...
    if !_revivePending
        ...
        ; BLEEDOUT-specific flavor: show the absorb VFX immediately, then delay
        ; the actual revive flow so the player lingers in bleedout with effects active.
        if context == "BLEEDOUT ENTER"
            LogMsg(LOG_DBG(), "BLEEDOUT: early dragon-soul VFX + hold=" + DragonSoulBleedoutVFXHoldSeconds)
            if DragonSoulVFXSpell
                LogMsg(LOG_DBG(), "BLEEDOUT: casting early DragonSoulVFXSpell")
                DragonSoulVFXSpell.Cast(player, player)
                _reviveEarlyVFXPlayed = True
            else
                LogMsg(LOG_DBG(), "BLEEDOUT: DragonSoulVFXSpell is None (early)")
            endif
            _reviveHoldUntil = Utility.GetCurrentRealTime() + DragonSoulBleedoutVFXHoldSeconds
        else
            _reviveHoldUntil = 0.0
            _reviveEarlyVFXPlayed = False
        endif
...
Bool Function HandleReviveFlow(Actor player)
    ...
    if _revivePending
        ...
        LogMsg(LOG_INFO(), "JOB D: state before consume: dead=" + player.IsDead() + " bleed=" + player.IsBleedingOut() + " consumed=" + _reviveConsumedSoul)

        ; If this revive was armed from BLEEDOUT, optionally hold the main
        ; revive sequence for a short time to let the early absorb VFX play
        ; while the player remains in bleedout.
        if _reviveHoldUntil > 0.0
            Float now = Utility.GetCurrentRealTime()
            if now < _reviveHoldUntil
                LogMsg(LOG_DBG(), "JOB D: holding revive for bleedout VFX. wait=" + (_reviveHoldUntil - now))
            _updateQueued = False
                QueueUpdate(_reviveHoldUntil - now)
            return True
            endif
            ; Hold period complete; clear and proceed.
            _reviveHoldUntil = 0.0
        endif
        ; If the engine (or another mod) pulled us out of bleedout before this job ran,
        ; we STILL complete the dragon-soul revive (consume soul + VFX) so the "extra life"
        ; is correctly accounted for and the player sees the feedback.
        if !player.IsDead() && !player.IsBleedingOut()
            LogMsg(LOG_INFO(), "JOB D: revive pending but player is already up; completing soul consume/VFX anyway")
        endif
        if !_reviveConsumedSoul
            ...
            if soulsNow > 0
                ...
                LogMsg(LOG_INFO(), "JOB D: soul consumed; now=" + soulsAfter)
                _reviveConsumedSoul = True
                if ShowDragonSoulPopup
                    LogMsg(LOG_DBG(), "JOB D: showing dragon-soul popup")
        ...
        if DragonSoulVFXSpell
            LogMsg(LOG_DBG(), "JOB D: casting DragonSoulVFXSpell")
            DragonSoulVFXSpell.Cast(player, player)
        else
            LogMsg(LOG_DBG(), "JOB D: DragonSoulVFXSpell is None")
        endif
        if ReviveStaggerSpell
            LogMsg(LOG_DBG(), "JOB D: casting ReviveStaggerSpell")
            ReviveStaggerSpell.Cast(player, player)
        else
            LogMsg(LOG_DBG(), "JOB D: ReviveStaggerSpell is None")
        endif
        if ReviveCloakSpell
            LogMsg(LOG_DBG(), "JOB D: casting ReviveCloakSpell")
            ReviveCloakSpell.Cast(player, player)
        else
            LogMsg(LOG_DBG(), "JOB D: ReviveCloakSpell is None")
        endif
