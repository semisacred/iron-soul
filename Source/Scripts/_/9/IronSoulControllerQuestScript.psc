...
Function TickRespawnCooldown(Actor player, String guid)
	...
	Int nowSec = Utility.GetCurrentRealTime() as Int
	Int lastSec = _GetAuthInt(player, "", GetCooldownLastKeyById(guid), nowSec)
...
Function HandlePlayerDying(Actor akKiller)
	...
	; No free respawn remaining.
	; If the player still has a dragon soul available, we KEEP them essential/protected so they don't ragdoll.
	; If there are no souls, we set non-essential so true death can occur.
	int souls = player.GetActorValue("DragonSouls") as int
	LogMsg(LOG_INFO(), "OnDying: no free respawn; DragonSouls=" + souls)

	if souls > 0
		; No free respawn quest should be running in this state.
		if RespawnQuest && RespawnQuest.IsRunning()
			RespawnQuest.Stop()
		endif
		; IMPORTANT: keep essential so the dragon-soul revive path never ragdolls.
		player.GetActorBase().SetEssential(True)
		if !_revivePending
	...
	; No respawn and no souls: allow a real death.
	if RespawnQuest && RespawnQuest.IsRunning()
		RespawnQuest.Stop()
	endif
