...
Float Property NoRespawnDeathCheckDelaySeconds = 12.0 Auto ; wait window for DragonsoulResurrect to revive before death triggers
Float Property LoadMessageDelay = 2.00 Auto
Float Property RespawnWarningDelaySeconds = 4.0 Auto
...
; Optional: DSR cleanup (fill these in CK only if you want Iron Soul to remove DSR assets during uninstall)
...
; Rule of thumb: section headers use the exact dashed style below.
; As many = as header text letters.
; =============
; SECTION TITLE
; =============
...
; Co-save (SKSE/StorageUtil) is used as a backup store for GUID and a small set of critical keys (e.g., deaths/respawn/cooldown/poem token),
; and is only consulted when JSON files are missing.
...
;This helper is called from GameReloaded() and is responsible only for
...
Function HandleBleedoutRespawn(Actor player)
    ; Called after bleedout exits to finalize the free-respawn flow.
    ; Dragon-soul revival is handled entirely by DragonsoulResurrect (not Iron Soul).
    if !player
        return
    endif
...
Bool Function HandleDisableRespawn(Actor player)
    ...
    if _pendingDisableRespawn
        ...
        if !player.IsBleedingOut() && !player.IsDead()
            ...
            ; After a free-respawn completes we are back in normal mortality.
            ; If we leave the player essential here, DragonsoulResurrect's OnDying path will
            ; see Target.IsEssential() and will NOT consume a dragon soul, leading to a soft-locky
            ; "glitch death" state on later deaths when no free respawn is available.
            if player
                player.GetActorBase().SetEssential(False)
                LogMsg(LOG_INFO(), "JOB B: cleared player essential after respawn completion")
            endif
...
Function HandlePlayerDying(Actor akKiller)
	...
	if _uninstallMode
		; In uninstall mode, do not intercept deaths.
		return
	endif
	...
	; Iron Soul no longer consumes dragon souls. Instead we:
	; 1) Ensure the player is NON-essential so DragonsoulResurrect can revive (if souls exist).
	; 2) Schedule a delayed death check. If the player is still dead after the DSR window,
	;    this is a true death and we log/exit as normal.
	if RespawnQuest && RespawnQuest.IsRunning()
		RespawnQuest.Stop()
	endif
	player.GetActorBase().SetEssential(False)
...
; When no free respawn is available, HandlePlayerDying arms a delayed check so
...
Bool Function HandlePendingDeathCheck(Actor player)
    ...
    ; Clear grace snapshot unless we confirm a real DSR consume below.
    ...
    ; Player survived the grace window. Only treat this as a valid DSR revive if a soul was actually consumed.
    Int soulsAfter = player.GetActorValue("DragonSouls") as Int
    if soulsBefore <= 0 || soulsAfter >= soulsBefore
        LogMsg(LOG_INFO(), "TRUE DEATH: player alive after grace but no soul was consumed (before=" + soulsBefore + " after=" + soulsAfter + "); exiting")
        TrueDeathAndQuit(player)
        return True
    endif
...
Function HandleUninstallMode(Actor player)
    ...
    if _uninstallStage == 2
        ...
        if DROnDying && player.HasSpell(DROnDying)
            player.RemoveSpell(DROnDying)
        endif
        if DRQuest
            DRQuest.Stop()
        endif
...
Bool Function BootstrapTick()
    ...
    _bootstrapActive = False
    LogMsg(LOG_INFO(), "Bootstrap: player ready; initial sync")
...
Function _UpdatePlayerEssentialState(Actor player, String guid)
    ; Desired rules (Iron Soul + DragonsoulResurrect compatibility):
    ; - Essential ONLY when a free respawn is available (to allow bleedout/respawn flow).
    ; - Non-essential when no respawn is available, so DragonsoulResurrect can consume souls and revive.
    if !player || guid == ""
        return
    endif

    Bool freeRespawn = _HasFreeRespawnAvailable(player, guid)
    Bool shouldEssential = freeRespawn
...
Bool Function HandleBleedoutDetection(Actor player)
    ...
    ; - If no free respawn is actually available, resolve the state immediately by
    ...
    if nowBleed && !_wasBleedingOut
        ...
        ; Update last known name (tooling/debug) if we have a stable GUID + name.
    ...
    if !nowBleed && _wasBleedingOut && _bleedoutArmed
        ...
        ; If respawn completed, run the respawn handler unless another job is pending.
        if !_pendingDisableRespawn
